...
<body>
    ...
    <div class="content">
        ...
        <div class="innerOuter" style="padding:5% 10%;">
					...
					<c:choose>
						<c:when test="${ empty condition }">
    		            	<c:if test="${ map.pi.currentPage gt 1 }">
            		        	<li class="page-item"><a class="page-link" href="/spring/boards?page=${ map.pi.currentPage -1 }">이전</a></li>
                   			</c:if>
                    
                    		<c:forEach var="num" begin="${ map.pi.startPage }" end="${ map.pi.endPage }">
                    			<li class="page-item">
                    				<a class="page-link" 
                    				   href="/spring/boards?page=${ num }">${ num }</a>
                    			</li>
		                    </c:forEach>
                    
                    		<c:if test="${ map.pi.currentPage lt map.pi.maxPage }">
                    			<li class="page-item"><a class="page-link" href="/spring/boards?page=${ map.pi.currentPage +1 }">다음</a></li>
                    		</c:if>
						</c:when>
						<c:otherwise>
    		            	<c:if test="${ map.pi.currentPage gt 1 }">
            		        	<li class="page-item">
            		        		<a class="page-link" 
            		        		   href="/spring/boards/keyword?page=${ map.pi.currentPage -1 }&condition=${condition}&keyword=${keyword}">
            		        			이전
            		        		</a>
            		        	</li>
                   			</c:if>
                    		...
                    		<c:forEach var="num" begin="${ map.pi.startPage }" end="${ map.pi.endPage }">
    	                		<li class="page-item">
    	                			<a class="page-link" 
    	                			   href="/spring/boards/keyword?page=${ num }&condition=${condition}&keyword=${keyword}">
    	                				${ num }
    	                			</a>
    	                		</li>
		                    </c:forEach>
            ...
            <form id="searchForm" action="/spring/boards/keyword" method="get" style="align : center;">
                <div class="select">
